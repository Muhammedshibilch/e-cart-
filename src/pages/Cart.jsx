...
import { emptyCart, incrementQuantity, removeCarItem } from '../redux/slice/cartSlice'
import { decrementQuantity } from '../redux/slice/cartSlice'
...
const Cart = () => {
  ...
  const handleDecrementQuantity =(product)=>{
    if(product?.quantity>1){
      ...
      dispatch(removeCarItem(product.id))
    }
  }
  const checkout =()=>{
    dispatch(emptyCart())
  ...
  userCart?.map((product,index)=>(
    ...
    <td>
      <div className="flex">
        ...
        <input type="text" style={{width:'40px'}} className="border p-1 rounded mx-2" readOnly value={product?.quantity} />
        <button onClick={()=>dispatch(incrementQuantity(product?.id))}  className="font-bold">+</button>

      </div>
    </td>
    <td>$ {product?.totalPrice}</td>
    <td><button onClick={()=>dispatch(removeCarItem(product?.id))} className="text-red-600"><i className="fa-solid fa-trash"></i></button></td>
  </tr>
  ))
}
      ...
      <div className="col-span-1">
        <div className="border rounded shadow p-5">
          <h2 className="text-2xl font-bold my-4">Total Amount :  <span className='text-red-600'> $ {cartTotal}</span></h2><hr />
